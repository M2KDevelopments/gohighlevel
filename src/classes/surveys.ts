...
export class Survey {
    ...
    */
    async getSubmissions(locationId: string, skip: number = 0, limit = 20, page = 1, search = "", type = "", startAt = "", endAt = "") {
        const headers = this.authData?.headers;
        const response = await axios.get(`${this.authData?.baseurl}/surveys/submissions/?locationId=${locationId}&limit=${limit}&page=${page}&skip=${skip}${type ? `&type=${type}` : ""}${startAt ? `&startAt=${startAt}` : ""}${endAt ? `&endAt=${endAt}` : ""}${search ? `&q=${search}` : ""}`, { headers });
        return response.data as {
